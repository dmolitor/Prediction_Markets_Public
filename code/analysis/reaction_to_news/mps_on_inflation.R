...
load_data_unemployment <- function() {
  ...
  df_kalshi <- read_csv('data/daily_moments_data_middle_out/daily_moments_headline_cpi_releases.csv')
...
responses_to_surprises <- function(df, output_name = 'output/mps_regressions.tex', spec = 'mps_statement') {
  ...
  if(spec == 'mps_statement') {
    note_text <- "Notes: Robust standard errors (HC3). Columns represent change in moments of the Inflation Rate distribution on the day of the news release of meetings more than 2 months away (end of previous day to end of day of release). A monetary policy shock is measured as the raw change in the fourth ED contract (in 2022) and the fourth 3 month SOFR contract (2023-2025). Statement shock from San Francisco Fed's USMPD Database."
  } else if(spec == 'mps_event') {
  ...
  modelsummary(
    ...
    title  = paste("Inflation Distribution responses to Monetary Policy Shocks"),
    notes = note_text,
    output = output_name                   
  )
...
run_exhibit <- function() {
  ...
  responses_to_surprises(df, spec = 'mps_statement', output_name = 'output/mps_regressions_statement_inflation.tex')
  responses_to_surprises(df, spec = 'mps_event', output_name = 'output/mps_regressions_event_inflation.tex')
